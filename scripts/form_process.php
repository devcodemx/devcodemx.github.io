...
function process_form($form) {
	if ($_SERVER['REQUEST_METHOD'] != 'POST')
		die(get_form_error_response('Método de solicitud de servidor desconocido'));

	if (formthrottle_too_many_submissions($_SERVER['REMOTE_ADDR']))
		die(get_form_error_response('Se han realizado recientemente demasiados envíos a través de esta IP'));
	...
	switch($properties['type']) {
		...
		case 'recaptcha':
			if (!array_key_exists('recaptcha', $form) || !array_key_exists('private_key', $form['recaptcha']) || empty($form['recaptcha']['private_key']))
				die(get_form_error_response('Clave privada de reCAPTCHA no válida.'));
		...
		default:
			die(get_form_error_response(sprintf('Tipo de campo desconocido: %s.', $properties['type'])));
	}
}
...
function email_form_submission($form) {
	...
	if(!$sent)
		die(get_form_error_response('Error al enviar el correo electrónico'));
...
function get_email_body($subject, $heading, $fields) {
	...
	$message .= '</table>';
	$message .= '<br/><br/>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">Formulario enviado desde el sitio web: ' . encode_for_form($_SERVER['SERVER_NAME']) . '</div>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">Dirección IP del visitante: ' . encode_for_form($_SERVER['REMOTE_ADDR']) . '</div>';
	$message .= '</body></html>';
...
function get_form_field_value($field, $properties) {
	...
	switch($properties['type']) {
		case 'checkbox':
			return (($value == '1' || $value == 'true') ? 'Marcada' : 'No marcada');
